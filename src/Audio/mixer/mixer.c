...
#define __MIXER_INTERNAL_EFFECT__
...
//typedef struct _Mixer_effectinfo
//{
//  Mixer_EffectFunc_t callback;
//  Mixer_EffectDone_t done_callback;
//  void *udata;
//  struct _Mixer_effectinfo *next;
//} effect_info;
...
int Mixer_GetNumChunkDecoders(void) {
  return (num_decoders);
}

const char *Mixer_GetChunkDecoder(int index) {
...
SDL_bool Mixer_HasChunkDecoder(const char *name) {
  int index;
...
//const SDL_version *Mixer_Linked_Version(void)
//{
...
//static int _Mixer_remove_all_effects(int channel, effect_info **e);

/*
 * rcg06122001 Cleanup effect callbacks.
 *  MAKE SURE Mixer_LockAudio() is called before this (or you're in the
 *   audio callback).
 */
static void _Mixer_channel_done_playing(int channel) {
   ...
   */
//  _Mixer_remove_all_effects(channel, &mix_channel[channel].effects);
}


static void *Mixer_DoEffects(int chan, void *snd, int len) {
  return (snd);
//  int posteffect = (chan == MIXER_CHANNEL_POST);
  ...
  for (i = 0; i < num_channels; ++i) {
    if (!mix_channel[i].paused) {
      if (mix_channel[i].expire > 0 && mix_channel[i].expire < sdl_ticks) {
        ...
        _Mixer_channel_done_playing(i);
      }
      else if (mix_channel[i].fading != MIXER_NO_FADING) {
        Uint32 ticks = sdl_ticks - mix_channel[i].ticks_fade;
        if (ticks >= mix_channel[i].fade_length) {
          Mixer_Volume(i, mix_channel[i].fade_volume_reset); /* Restore the volume */
          if (mix_channel[i].fading == MIXER_FADING_OUT) {
            mix_channel[i].playing = 0;
            mix_channel[i].looping = 0;
            mix_channel[i].expire = 0;
            _Mixer_channel_done_playing(i);
          ...
          if (mix_channel[i].fading == MIXER_FADING_OUT) {
            Mixer_Volume(i, (mix_channel[i].fade_volume * (mix_channel[i].fade_length - ticks))
                            / mix_channel[i].fade_length);
          }
          else {
            Mixer_Volume(i, (mix_channel[i].fade_volume * ticks) / mix_channel[i].fade_length);
          }
        }
      }
      if (mix_channel[i].playing > 0) {
        ...
        while (mix_channel[i].playing > 0 && index < len) {
          remaining = len - index;
          volume = (mix_channel[i].volume * mix_channel[i].chunk->volume) / MIXER_MAX_VOLUME;
...
//      Mixer_UnregisterAllEffects(i);
  ...
  Mixer_LockAudio();
  mix_channel = (struct _Mixer_Channel *) SDL_realloc(mix_channel, numchans * sizeof(struct _Mixer_Channel));
  if (numchans > num_channels) {
    ...
    for (i = num_channels; i < numchans; i++) {
      ...
      mix_channel[i].playing = 0;
      mix_channel[i].looping = 0;
      mix_channel[i].volume = MIXER_MAX_VOLUME;
      mix_channel[i].fade_volume = MIXER_MAX_VOLUME;
      mix_channel[i].fade_volume_reset = MIXER_MAX_VOLUME;
      mix_channel[i].fading = MIXER_NO_FADING;
      mix_channel[i].tag = -1;
      mix_channel[i].expire = 0;
//      mix_channel[i].effects = NULL;
...
int Mixer_QuerySpec(int *frequency, Uint16 *format, int *channels) {
  if (audio_opened) {
    if (frequency) {
...
//{
//  int i;
//  Mixer_MusicInterface *interface = NULL;
//  void *music = NULL;
//  Sint64 start;
//  SDL_bool playing;
...
//    if (interface->api == MIXER_MUSIC_CMD ||
//       interface->api == MIXER_MUSIC_MIKMOD ||
//       interface->api == MIXER_MUSIC_NATIVEMIDI) {
//      continue;
//    }
//
...
//      SDL_RWclose(src);
//    }
//    Mixer_SetError("Unrecognized audio format");
//    return NULL;
//  }
//
//  Mixer_LockAudio();
...
Mixer_Chunk *Mixer_LoadWAV_RW(SDL_RWops *src, int freesrc) {
  ...
  if (!src) {
    SDL_SetError("Mixer_LoadWAV_RW with NULL src");
    return (NULL);
  }
  ...
  chunk = (Mixer_Chunk *) SDL_malloc(sizeof(Mixer_Chunk));
  if (chunk == NULL) {
  ...
  if (SDL_RWread(src, magic, 1, 4) != 4) {
    ...
    Mixer_SetError("Couldn't read first 4 bytes of audio data");
    return NULL;
  }
...
//    loaded = Mixer_LoadMusic_RW(music_type, src, freesrc, &wavespec, (Uint8 **)&chunk->abuf, &chunk->alen);
  ...
  chunk->allocated = 1;
  chunk->volume = MIXER_MAX_VOLUME;

  return (chunk);
}

/* Load a wave file of the mixer format from a memory buffer */
Mixer_Chunk *Mixer_QuickLoad_WAV(Uint8 *mem) {
  Mixer_Chunk *chunk;
  Uint8 magic[4];
  ...
  /* Allocate the chunk memory */
  chunk = (Mixer_Chunk *) SDL_calloc(1, sizeof(Mixer_Chunk));
  if (chunk == NULL) {
    SDL_SetError("Out of memory");
    return (NULL);
  }
...
Mixer_Chunk *Mixer_QuickLoad_RAW(Uint8 *mem, Uint32 len) {
  Mixer_Chunk *chunk;
...
void Mixer_FreeChunk(Mixer_Chunk *chunk) {
  int i;
...
void *Mixer_GetMusicHookData(void) {
  return (music_data);
}

void Mixer_ChannelFinished(void (*channel_finished)(int channel)) {
  Mixer_LockAudio();
 ...
 */
int Mixer_ReserveChannels(int num) {
  if (num > num_channels)
    num = num_channels;
...
static int checkchunkintegral(Mixer_Chunk *chunk) {
  int frame_width = 1;
...
*/
int Mixer_PlayChannelTimed(int which, Mixer_Chunk *chunk, int loops, int ticks) {
  int i;
//  printf("Mixer_PlayChannelTimed (which %i, chunk %p, loops %i, ticks %i)\n", which, chunk, loops, ticks);
  /* Don't play null pointers :-) */
  if (chunk == NULL) {
    Mixer_SetError("Tried to play a NULL chunk");
    return (-1);
  }
  if (!checkchunkintegral(chunk)) {
    Mixer_SetError("Tried to play a chunk with a bad frame");
    return (-1);
  }
  ...
  Mixer_LockAudio();
  {
    ...
    if (which == -1) {
      ...
      if (i == num_channels) {
        Mixer_SetError("No free channels available");
...
//      printf("channel %i -> vol %i\n", i, mix_channel[i].volume);
//    }
    /* Queue up the audio data for this channel */
    if (which >= 0 && which < num_channels) {
      Uint32 sdl_ticks = SDL_GetTicks();
      if (Mixer_Playing(which)) {
        _Mixer_channel_done_playing(which);
      }
      ...
      mix_channel[which].fading = MIXER_NO_FADING;
...
int Mixer_ExpireChannel(int which, int ticks) {
  int status = 0;

  if (which == -1) {
    int i;
    for (i = 0; i < num_channels; ++i) {
      status += Mixer_ExpireChannel(i, ticks);
    }
  }
...
int Mixer_FadeInChannelTimed(int which, Mixer_Chunk *chunk, int loops, int ms, int ticks) {
  int i;
  ...
  Mixer_LockAudio();
  {
    ...
    if (which >= 0 && which < num_channels) {
      ...
      mix_channel[which].chunk = chunk;
      mix_channel[which].paused = 0;
      mix_channel[which].fading = MIXER_FADING_IN;
...
int Mixer_Volume(int which, int volume) {
  int i;
  int prev_volume = 0;

  if (which == -1) {
    for (i = 0; i < num_channels; ++i) {
      prev_volume += Mixer_Volume(i, volume);
    }
...
int Mixer_VolumeChunk(Mixer_Chunk *chunk, int volume) {
  int prev_volume;
...
int Mixer_HaltChannel(int which) {
  int i;
  ...
  if (which == -1) {
    ...
    if (mix_channel[which].fading != MIXER_NO_FADING) /* Restore volume */
...
int Mixer_FadeOutChannel(int which, int ms) {
  int status;
  ...
  if (audio_opened) {
    if (which == -1) {
      ...
      for (i = 0; i < num_channels; ++i) {
        status += Mixer_FadeOutChannel(i, ms);
      }
    }
    else if (which < num_channels) {
      Mixer_LockAudio();
      if (mix_channel[which].playing &&
          (mix_channel[which].volume > 0) &&
          (mix_channel[which].fading != MIXER_FADING_OUT)) {
        mix_channel[which].fade_volume = mix_channel[which].volume;
        mix_channel[which].fading = MIXER_FADING_OUT;
        mix_channel[which].fade_length = (Uint32) ms;
        ...
        if (mix_channel[which].fading == MIXER_NO_FADING) {
          mix_channel[which].fade_volume_reset = mix_channel[which].volume;
        }
        ++status;
      }
...
/* Halt playing of a particular group of channels */
int Mixer_FadeOutGroup(int tag, int ms) {
  int i;
  int status = 0;
  for (i = 0; i < num_channels; ++i) {
    if (mix_channel[i].tag == tag) {
...
Mixer_Fading Mixer_FadingChannel(int which) {
  if (which < 0 || which >= num_channels) {
    return MIXER_NO_FADING;
  }
...
Mixer_Chunk *Mixer_GetChunk(int channel) {
  Mixer_Chunk *retval = NULL;
...
int Mixer_GroupChannels(int from, int to, int tag) {
  int status = 0;
  for (; from <= to; ++from) {
    status += Mixer_GroupChannel(from, tag);
  }
  return (status);
}

/* Finds the first available channel in a group of channels */
int Mixer_GroupAvailable(int tag) {
  int i;
  for (i = 0; i < num_channels; i++) {
...
/* MAKE SURE you hold the audio lock (Mixer_LockAudio()) before calling this! */
//static int _Mixer_register_effect(effect_info **e, Mixer_EffectFunc_t f,
//        Mixer_EffectDone_t d, void *arg)
//{
//  effect_info *new_e;
//
//  if (!e) {
//    Mixer_SetError("Internal error");
//    return(0);
//  }
//
//  if (f == NULL) {
//    Mixer_SetError("NULL effect callback");
//    return(0);
//  }
//
//  new_e = SDL_malloc(sizeof (effect_info));
//  if (new_e == NULL) {
//    Mixer_SetError("Out of memory");
//    return(0);
//  }
//
//  new_e->callback = f;
...
//
///* MAKE SURE you hold the audio lock (Mixer_LockAudio()) before calling this! */
//static int _Mixer_remove_effect(int channel, effect_info **e, Mixer_EffectFunc_t f)
//{
//  effect_info *cur;
...
//  }
//
//  Mixer_SetError("No such effect registered");
//  return(0);
//}
...
/* MAKE SURE you hold the audio lock (Mixer_LockAudio()) before calling this! */
//int _Mixer_RegisterEffect_locked(int channel, Mixer_EffectFunc_t f,
//      Mixer_EffectDone_t d, void *arg)
//{
//  effect_info **e = NULL;
//
//  if (channel == MIXER_CHANNEL_POST) {
//    e = &posteffects;
//  } else {
//    if ((channel < 0) || (channel >= num_channels)) {
//      Mixer_SetError("Invalid channel number");
//      return(0);
//    }
//    e = &mix_channel[channel].effects;
//  }
//
//  return _Mixer_register_effect(e, f, d, arg);
//}

//int Mixer_RegisterEffect(int channel, Mixer_EffectFunc_t f,
//      Mixer_EffectDone_t d, void *arg)
//{
//  int retval;
//  Mixer_LockAudio();
//  retval = _Mixer_RegisterEffect_locked(channel, f, d, arg);
//  Mixer_UnlockAudio();
//  return retval;
//}
//
//
///* MAKE SURE you hold the audio lock (Mixer_LockAudio()) before calling this! */
//int _Mixer_UnregisterEffect_locked(int channel, Mixer_EffectFunc_t f)
//{
//  effect_info **e = NULL;
//
//  if (channel == MIXER_CHANNEL_POST) {
...
//  }
//
//  return _Mixer_remove_effect(channel, e, f);
//}
//
//int Mixer_UnregisterEffect(int channel, Mixer_EffectFunc_t f)
//{
//  int retval;
//  Mixer_LockAudio();
//  retval = _Mixer_UnregisterEffect_locked(channel, f);
//  Mixer_UnlockAudio();
//  return(retval);
//}

/* MAKE SURE you hold the audio lock (Mixer_LockAudio()) before calling this! */
//int _Mixer_UnregisterAllEffects_locked(int channel)
//{
//  effect_info **e = NULL;
//
//  if (channel == MIXER_CHANNEL_POST) {
...
//  }
//
//  return _Mixer_remove_all_effects(channel, e);
//}

//int Mixer_UnregisterAllEffects(int channel)
//{
//  int retval;
//  Mixer_LockAudio();
//  retval = _Mixer_UnregisterAllEffects_locked(channel);
//  Mixer_UnlockAudio();
//  return(retval);
//}
